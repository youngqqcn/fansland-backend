...
fn verify_signature(msg: String, sig: String, address: String) {
    // 将字符串签名转换为 Signature 类型
    let signature = Signature::from_str(&sig).unwrap();
    let address = Address::from_str(&address).unwrap();

    signature.verify(msg.clone(), address).unwrap();

    let addr = signature.recover(msg.clone()).unwrap();
...
fn main() {
    ...
    // 将字节数组转换为字符串
    // let decoded_string = String::from_utf8(decoded_bytes).expect("Failed to convert bytes to string");
    // let x = hex
